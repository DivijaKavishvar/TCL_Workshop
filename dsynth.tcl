#! /bin/env tclsh
puts "\n\n=============================================================="
puts "                    WELCOME TO DSYNTH"
puts "--------------------------------------------------------------"
puts "  An Open-Source Synthesis & STA Automation Tool"
puts "  \tDeveloped by Divija Kavishvar"
puts "==============================================================\n"

#----- Checks whether dsynth usage is correct or not -----#
set enable_prelayout_timing 1
set working_dir [exec pwd]
set dsynth_array_length [llength [split [lindex $argv 0] .]]
set input [lindex [split [lindex $argv 0] .] $dsynth_array_length-1]
if {![regexp {^csv} $input] || $argc!=1 } {
	puts "DSYNTH Error: Incorrect usage."
	puts "Usage: ./dsynth <.csv>"
	...
	while {$i < $num_of_rows} {
		 puts "\nDSYNTH Info: Setting $my_arr(0,$i) as '$my_arr(1,$i)'"
		 if {$i == 0} {
...
puts "\nDSYNTH Info: Below are the list of initial variables and their values:"
...
if {! [file isdirectory $OutputDirectory]} {
	puts "\nDSYNTH Info: Creating output directory $OutputDirectory"
	file mkdir $OutputDirectory
} else {
	puts "\nDSYNTH Info: Output directory found at $OutputDirectory"
}

if {! [file isdirectory $NetlistDirectory]} {
	puts "\nDSYNTH Error: RTL netlist directory not found at $NetlistDirectory. Exiting..."
	exit
} else {
	puts "\nDSYNTH Info: RTL netlist directory found at $NetlistDirectory"
}
if {! [file exists $EarlyLibraryPath]} {
	puts "\nDSYNTH Error: Early library not found at $EarlyLibraryPath. Exiting..."
	exit
} else {
	puts "\nDSYNTH Info: Early cell library found at $EarlyLibraryPath"
}

if {! [file exists $LateLibraryPath]} {
	puts "\nDSYNTH Error: Late library not found at $LateLibraryPath. Exiting..."
	exit
} else {
	puts "\nDSYNTH Info: Late cell library found at $LateLibraryPath"
}

if {! [file exists $ConstraintsFile]} {
	puts "\nDSYNTH Error: Constraints file not found at $ConstraintsFile. Exiting..."
	exit
} else {
	puts "\nDSYNTH Info: Constraints file found at $ConstraintsFile"
}
...
puts "\nDSYNTH Info: Dumping SDC constraints for $DesignName"
...
set formatStr {%10s%15s%15s%15s%15s%15s%15s%10s%10s}
